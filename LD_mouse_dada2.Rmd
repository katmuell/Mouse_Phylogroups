...
```{r}
#Files
map.file = file.path(data.dir, "LD_mouse_map.txt")
silva.ref = file.path(data.dir, "silva_nr99_v138.1_wSpecies_train_set.fa.gz")
ps.rds = file.path(data.dir, "LD_mice.rds")
```
...
Filter reads
```{r}
filt.out <- filterAndTrim(fnFs, filtFs, fnRs, filtRs, trimLeft = 10, truncLen = c(240, 240),
...
```{r}
seqtab2 <- seqtab[, nchar(colnames(seqtab)) %in% seq(418,448)]
```
