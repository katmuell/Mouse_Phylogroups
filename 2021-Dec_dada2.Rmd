...
demuc.redo.dir = "/work/kdm65/16S_Mar_2022"
...
Load metadata
```{r}
metadata.df = read_tsv(redo.map.file)
...
otus = otu_table(seqtab.nochim, taxa_are_rows = FALSE)
sd = sample_data(metadata.df)
redo.ps <- phyloseq(otus, sd, tax_table(taxa))
...
```{r}
#Load initial object
loaded.initial.ps = read_rds(ps.rds)

print(loaded.initial.ps)
```

```{r}
#Merge phyloseq objects
merged.ps <- merge_phyloseq(loaded.initial.ps, loaded.redo.ps)
